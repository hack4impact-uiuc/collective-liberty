...
const preprocessCSV = (incident) => {
  ...
  let stateCounts = {}; // key-value pairs of state->incident count
  let cityCounts = {}; // key-value pairs of city->incident count
  let yearCounts = {}; // key-value pairs of year(19,20...)->incident count
  let incidentCounts = {}; // key-value pairs of incident type(human trafficking, massage parlor...)-incident count

  if(stateCounts[incident["Business State"]] == null || stateCounts[incident["Business State"]] == undefined)
    stateCounts[incident["Business State"]] = 1;
  else
    stateCounts[incident["Business State"]] = stateCounts[incident["Business State"]] + 1;

  if(cityCounts[incident["Business City"]] == null || cityCounts[incident["Business City"]] == undefined)
  ...
  if(yearCounts[date] == null || yearCounts[date] == undefined)
    yearCounts[date] = 1;
  else
    yearCounts[date] = yearCounts[date] + 1;

  if(incidentCounts[incident["Content/Focus"]] == null || incidentCounts[incident["Content/Focus"]] == undefined)
